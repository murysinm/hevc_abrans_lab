...
uint32_t g_abrans_recip[ABRANS_PROB_SCALE];
...
void abrans_init_tables()
{
    ...
    g_abrans_recip[0] = 0;
    for (int p = 1; p < ABRANS_PROB_SCALE; p++)
        g_abrans_recip[p] = (uint32_t)(((uint64_t)1 << 30) / p);
    initialized = true;
}
...
Entropy::Entropy()
{
    ...
    abrans_init_tables();
    for (int i = 0; i < ABRANS_NUM_CTX; i++)
        m_abransVsw[i] = ABRANS_VSW_HALF;
    for (int i = 0; i < ABRANS_NUM_CTX; i++)
        m_abransCarryOverVsw[i] = ABRANS_VSW_HALF;
    m_abransCarryOver = false;
    m_isISlice = false;
}
...
void Entropy::copyFrom(const Entropy& src)
{
    ...
    memcpy(m_abransVsw, src.m_abransVsw, sizeof(m_abransVsw));
    m_isISlice = src.m_isISlice;  
    markValid();
}
...
void Entropy::encodeBin(uint32_t binValue, uint8_t &ctxModel)
{
    ...
    ptrdiff_t context_index = &ctxModel - m_contextState;
    m_abransBuf.push_back({(uint8_t)binValue, (int16_t)context_index});
    return;
}
...
void Entropy::encodeBinEP(uint32_t binValue)
{
    ...
    m_abransBuf.push_back({(uint8_t)binValue, (int16_t)-1});
}
...
void Entropy::encodeBinsEP(uint32_t binValues, int numBins)
{
    ...
    while (numBins != 0) {
        encodeBinEP((binValues >> (numBins - 1)) & 1);
        --numBins;
    }
}
...
void Entropy::encodeBinTrm(uint32_t binValue)
{
    ...
    m_abransBuf.push_back({(uint8_t)binValue, (int16_t)-2});
}
...
void Entropy::abransReset()
{
    m_abransBuf.clear();
    for (int i = 0; i < 160; i++)
        m_abransVsw[i] = abrans_vsw_from_cabac(m_contextState[i]);
    m_abransVsw[160] = ABRANS_VSW_HALF;
}

void Entropy::abransFlush()
{
    size_t N = m_abransBuf.size();
    if (N == 0) return;

    std::vector<uint16_t> states(N);
    uint16_t vsw[ABRANS_NUM_CTX];
    {
        const uint16_t* init = (m_abransCarryOver && !m_isISlice) ? m_abransCarryOverVsw : m_abransVsw;
        for (int i = 0; i < ABRANS_NUM_CTX; i++)
            vsw[i] = init[i];
    }
    ...
    {
        int16_t ctx = m_abransBuf[i].ctxIdx;
        uint8_t bit  = m_abransBuf[i].bit;

        if (ctx == -1)
        {
            states[i] = (uint16_t)(ABRANS_PROB_SCALE / 2);
        }
        else
        {
            int vswIdx = (ctx == -2) ? 160 : (int)ctx;
            uint32_t p1 = (uint32_t)vsw[vswIdx] << ABRANS_WSHIFT;
            if (p1 == 0) p1 = 1;
            if (p1 >= (uint32_t)ABRANS_PROB_SCALE) p1 = ABRANS_PROB_SCALE - 1;
            states[i] = (uint16_t)p1;
            if (bit == 0)
                vsw[vswIdx] -= vsw[vswIdx] >> ABRANS_VSW_LEN;
            else
                vsw[vswIdx] += (uint16_t)((ABRANS_VSW_ONE - vsw[vswIdx]) >> ABRANS_VSW_LEN);
        }
    }

    for (int i = 0; i < ABRANS_NUM_CTX; i++)
        m_abransCarryOverVsw[i] = vsw[i];
    m_abransCarryOver = true;

    size_t out_max = N + 32;
    std::vector<uint8_t> out_buf(out_max);
    uint8_t* ptr = out_buf.data() + out_max;
    uint32_t rans = ABRANS_RANS_BYTE_L;

    for (size_t i = N; i > 0; i--)
    {
        uint8_t s    = m_abransBuf[i - 1].bit;
        uint32_t p1  = states[i - 1];
        uint32_t p0  = ABRANS_PROB_SCALE - p1;
        uint32_t p   = s ? p1 : p0;
        uint32_t off = s ? p0 : 0;
        if (rans >= p << ABRANS_XMAX_SHIFT) { *--ptr = (uint8_t)(rans & 0xff); rans >>= 8; }
        uint32_t recip = g_abrans_recip[p];
        uint32_t q = (uint32_t)(((uint64_t)rans * recip) >> 30);
        uint32_t r = rans - q * p;
        if (r >= p) { q++; r -= p; }
        if (r >= p) { q++; r -= p; }
        rans = (q << ABRANS_PROB_BITS) + off + r;
    }
    ...
    uint32_t compressed_size = (uint32_t)(out_buf.data() + out_max - ptr);

    uint32_t n = (uint32_t)N;
    for (int shift = 0; shift < 32; shift += 8)
        m_bitIf->write((n >> shift) & 0xff, 8);
    for (int shift = 0; shift < 32; shift += 8)
        m_bitIf->write((compressed_size >> shift) & 0xff, 8);

    for (uint32_t j = 0; j < compressed_size; j++)
        m_bitIf->write(ptr[j], 8);
